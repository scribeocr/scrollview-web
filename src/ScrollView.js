import { SVWindow } from './SVWindow.js';
...
export class ScrollView {
  ...
  writeAll(fileBase) {
    ...
    for (let key in this.windows) {
      if (this.windows[key].hasOwnProperty('name')) {
    ...
    for (const [key, value] of Object.entries(this.windows)) {
      ...
      const filenameSuffix = nameCount[name] > 1 ? '_' + String(nameCurrent[name]) : '';

      this.windows[key].writeCanvas(this.windows[key].canvas, `${fileBase}_${this.windows[key].name || key}${filenameSuffix}.png`);
    }
  }
      ...
      if (colon > 1 && colon < start) {
        ...
        switch (func) {
          ...
          case 'zoomRectangle':
            ...
            break;
          case 'readImage':
            this.windows[windowID].readImage();
            // Assuming SVImageHandler.readImage is adapted to JavaScript
            // let image = SVImageHandler.readImage(intList[2], in); // 'in' needs to be defined
